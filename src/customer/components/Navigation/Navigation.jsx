...
export default function Navigation() {
  ...
  const [searchInput, setSearchInput] = useState('');
    ...
    // --- NEW: Handler for initiating search ---
    const handleSearch = () => {
      if (searchInput.trim()) { // Only dispatch if search input is not empty
        console.log("Dispatching search for query:", searchInput);
        dispatch(searchProducts(searchInput.trim())); // Dispatch the searchProducts action
        // Optionally, you might want to clear the search input after dispatching:
        // setSearchInput('');
      } else {
        console.log("Search input is empty, not dispatching.");
      }
    };
    // You can also add a handler for "Enter" key press in the input field
  ...
  return (
    <div className="bg-white fixed top-0 left-0 w-full z-50 shadow-md">
      ...
      <Transition.Root show={open} as={Fragment}>
        <Dialog as="div" className="relative hidden" onClose={setOpen}> {/* Removed lg:hidden */}
          <Transition.Child
            as={Fragment}
        ...
        <nav aria-label="Top" className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="border-b border-gray-200">
            <div className="flex h-16 items-center">
              <button
                type="button"
                className="relative rounded-md bg-white p-2 text-gray-400 hidden" 
              ...
              {/* Logo */}
              <div className="ml-4 flex"> {/* Removed lg:ml-0 */}
                <a href="#">
                                  ...
                                  <div className="grid grid-cols-2 gap-x-8 gap-y-10 py-16">
                                    <div className="col-start-2 grid grid-cols-2 gap-x-8">
                                      {category.featured.map((item) => (
                                        <div 
                                            key={item.name} 
                                            className="group relative text-base sm:text-sm"> {/* Retained sm:text-sm as it's a sizing adjustment */}
                                          <div className="aspect-h-1 aspect-w-1 overflow-hidden rounded-lg bg-gray-100 group-hover:opacity-75">
                                            <img
                                              src={item.imageSrc}
                                              alt={item.imageAlt}
                ...
                {/* <div className="flex lg:ml-6">
                  <p href="#" className="p-2 text-gray-400 hover:text-gray-500">
                    ...
                    <MagnifyingGlassIcon className="h-6 w-6" aria-hidden="true" />
                                ...
                                {/* --- MODIFIED: Search input and button activation --- */}
                   <div className="flex items-center lg:ml-6 space-x-2"> {/* Added space-x-2 for spacing */}
                  <input
                    type="text"
                    placeholder="Search..."
                    ...
                    onKeyPress={handleKeyPress} // Allows searching with Enter key
                  />
                  <Button
